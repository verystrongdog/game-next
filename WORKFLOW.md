...
1. Accepted current-design documents and accepted ADRs.
2. The approved revision of the active Issue, for the local change it governs.
3. Executable evidence: tests, builds, recordings, saves, and playtest results.
4. Discussion threads and raw notes.
5. The legacy repository and its documents.
...
1. **Playable outcome over administrative output.** A collection of correct
   ...
   the current playable milestone or produces evidence needed to choose it.
2. **Raw discussion is not an implementation order.** Ideas, chat, legacy
   documents, screenshots, and comments must pass through the applicable Issue
   process before production implementation begins.
3. **Strictness follows uncertainty.** Bugs are strict about reproduction;
...
4. **Do not manufacture certainty.** Unknown game feel, balance, and experience
...
5. **One fact, one canonical home.** An Issue describes one proposed change or
...
6. **Approval binds an exact specification revision.** A substantive edit to an
   approved Issue invalidates admission and returns it to review.
7. **Evidence beats reports.** Agent summaries and checklist claims are not proof.
   Inspect the repository, run the checks, and play the relevant build.
8. **Limit work in progress.** Do not admit work merely because it is well
   specified. It must have a relationship to the current playable milestone and
...
| --- | --- | --- |
| Issue intake, clarification, admission, and revision | Active v0.1 | [`docs/workflow/issues.md`](docs/workflow/issues.md) |
| Pull requests and code review | Not yet defined | Future module |
| Automated verification and CI | Not yet defined | Future module |
| Playtest and acceptance | Not yet defined | Future module |
| Release and build retention | Not yet defined | Future module |
| Current-design and ADR maintenance | Not yet defined | Future module |
...
typed Issue draft
        |
        v
clarification -> owner confirmation -> adversarial admission review
        |                                  |
        |                                  +--> blocked / rejected
        v
...
experiment or implementation
        |
        v
playable verification and evidence
        |
        v
accept / revise / abandon
```
